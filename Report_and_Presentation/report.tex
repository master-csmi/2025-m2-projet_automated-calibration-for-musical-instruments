...
\begin{equation}
v(L,t) = \frac{\beta}{Z^\ast T^\ast} p(L,t) - \sqrt{\alpha}\,\phi(t).
\tag{BC}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        w^-
        &= p - v \\
        &= p - \left( \frac{\beta}{Z^\ast T^\ast} p - \sqrt{\alpha}\,\phi \right) \\
        &= \left(1 - \frac{\beta}{Z^\ast T^\ast}\right)p + \sqrt{\alpha}\,\phi.
    \end{aligned}
\end{equation}
...
\begin{equation}
w^-
= \left(1 - \frac{\beta}{Z^\ast T^\ast}\right)\frac{w^+ + w^-}{2} + \sqrt{\alpha} \phi.
\end{equation}
...
\boxed{
w^- =
\frac{1 - \frac{\beta}{Z^\ast T^\ast}}{1 + \frac{\beta}{Z^\ast T^\ast}}
w^+
+
\frac{2\sqrt{\alpha}}{1 + \frac{\beta}{Z^\ast T^\ast}} \phi
}
...
integration scheme as the DG solver.
...
\For{$n = 0, \dots, N_t-1$}
    ...
    \[
        w^+ = p_h(L^-) + v_h(L^-)
    \]
    \State \textbf{2. Compute incoming wave (boundary condition):}
    \[
        w^- = a\, w^+ + b\, \phi^n
    \]
    where
    \[
        a = \frac{1 - \beta/(Z^* T^*)}{1 + \beta/(Z^* T^*)}, \quad
        b = \frac{2 \sqrt{\alpha}}{1 + \beta/(Z^* T^*)}.
    \]
    \State \textbf{3. Reconstruct ghost state at the boundary:}
    \[
        p^+ = \frac{w^+ + w^-}{2},\quad
        v^+ = \frac{w^+ - w^-}{2}
    \]
    \State \textbf{4. Compute numerical flux at the boundary:}
    \[
        \hat{\mathbf F}_L = \text{Rusanov}(u^-, u^+)
    \]
    \State \textbf{5. Compute internal fluxes:}
    \For{each interior interface $j+1/2$}
        \State Compute $\hat{\mathbf F}_{j+1/2}$
    \EndFor
    \State \textbf{6. DG update (Euler or RK2):}
    \[
        M_k \, u_k^{\,n+1} = b_k(u^n, \hat{\mathbf F})
    \]
    \State \textbf{7. Update boundary ODE:}
    \[
        \phi^{\,n+1} = \phi^n - \Delta t \, \frac{T^*}{Z^*} \, \sqrt{\alpha} \, p_h(L^-)
    \]
\EndFor
\end{algorithmic}
\end{algorithm}
...
\section{Analytical solving when $S(x) = S^* = 1$}
...
\end{align}
\subsection{Boundary condition at $x=L$}
...
\begin{equation}
a=\frac{1-\beta/(Z^\ast T^\ast)}{1+\beta/(Z^\ast T^\ast)}, \qquad
b=\frac{2\sqrt{\alpha}}{1+\beta/(Z^\ast T^\ast)}, \qquad
c_1=\frac{\sqrt{\alpha}}{2Z^\ast T^\ast}, \qquad
...
We will fist describe the initial conditions and simulation parameters used in the experiments.
...
with an analyze of the convergence of the method for the uniform cross-section case. 
...
\hline
Parameters & $\alpha$ & $\beta$ & $S^\ast$ & c & $Z^\ast$ & $T^\ast$ & CLF \\
\hline
Values & 0.1 & 0.2 & 1 & 1 & 1.0 & 1.0 & 0.05 \\
\hline
\end{tabular}
...
Future work will focus on extending the solver to include the full right-hand-side boundary conditions described by equations~\eqref{eq:1e} 
and~\eqref{eq:1f}, which are essential for modeling the complete reed–instrument interaction. In addition, further investigations will be 
required to design an effective automated calibration strategy, including data preprocessing, parameter selection, and the choice of 
appropriate loss functions for optimization.
