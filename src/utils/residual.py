...
from utils.util_func import pressure_func as F_func
from numerics.dg.mesh import cell_edges_from_nodes
...
def compute_total_residual(
    ...
    y_snaps,      # (nsnaps,)
    z_snaps,      # (nsnaps,)  ∂_t y stocké directement
    phi_snaps,    # (nsnaps,)
    gamma_snaps,  # (nsnaps,)
    S_cells,      # (N,)
    S_star,
    ...
    # --------------------------------------------------
    pL = (c * S_star / S_L) * u[:, 0,  0, 0]   # p au bord gauche ✓
    vL = (c * S_L   / S_star) * u[:, 0,  1, 0]  # v au bord gauche ✓
    pR = (c * S_star / S_R) * u[:, -1, 0, 1]    # p au bord droit ✓
    vR = (c * S_R   / S_star) * u[:, -1, 1, 1]  # v au bord droit ✓
    ...
    # ∂_t p_tilde et ∂_t v_tilde au centre des cellules intérieures
    ...
    # -v(0,t) + zeta l(y) F(gamma-p) + eps κ/wr d_t y = 0
    # --------------------------------------------------
    res_bcL = jnp.abs(
        - vL
