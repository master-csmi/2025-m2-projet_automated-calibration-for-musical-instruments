...
\begin{equation}
M \, \boldsymbol{\alpha}^{n+1}_k = \mathbf{b}_k^n,
\label{eq:matrix_form}
\end{equation}
where $\mathbf{b}_k^n$ contains the volume and numerical flux contributions.

\section{Discontinuous Galerkin method in our context}

\subsection{Quasi - Conservative form}
...
\end{pmatrix},
\end{equation}
By introducing the transform variables : 
\begin{equation}
\mathbf{\tilde{u}} =
\begin{pmatrix}
\tilde{p}  \\
...
\begin{equation}
\tilde{p}(x,0) = \tilde{p_0}(x) = \frac{S(x)}{c S^\ast} p_0(x), \qquad \tilde{v}(x,0) = \tilde{v_0}(x) = \frac{c S^\ast}{S(x)} v_0(x).
\end{equation}

\subsection{Change of Variables for Uniform Mass Matrices}

To avoid variable mass matrices while preserving the conservative structure,
we introduce the following change of variables:
\begin{equation}
    \tilde{p} = \frac{S(x)}{cS^\ast}\, p, \qquad
    \tilde{v} = \frac{cS^\ast}{S(x)}\, v.
\end{equation}
Under this transformation, the system~\eqref{eq:conservative_form} becomes
\begin{equation}
    \partial_t \tilde{p} + \partial_x\!\left(\frac{S(x)}{cS^\ast}\tilde{v}\right) = 0,
    \qquad
    \partial_t \tilde{v} + \partial_x\!\left(\frac{cS^\ast}{S(x)}\tilde{p}\right) = 0.
\end{equation}
The time-derivative coefficients are now unity, so the local mass matrices reduce to
the standard P1 mass matrix
\begin{equation}
    M = \frac{h}{6}\begin{pmatrix}2&1\\1&2\end{pmatrix},
\end{equation}
independent of $S(x)$. This ensures a uniform CFL condition and simplifies the
inversion of the mass matrix. The wave speed $c$ remains the unique characteristic
speed, independently of the cross-section profile.
...
\subsection{Rusanov Flux for Variable Cross-Section}

In the $(\tilde p, \tilde v)$ variables, the numerical flux at interface
$x_{j+\frac12}$ with local section $S_{j+\frac12}=\frac12(S_j+S_{j+1})$ reads
\begin{equation}
    \widehat{\mathbf{F}} =
    \frac{1}{2}\bigl(\mathbf{F}^- + \mathbf{F}^+\bigr)
    - \frac{1}{2}\bigl(\mathbf{U}^+ - \mathbf{U}^-\bigr),
    \label{eq:rusanov_tilde}
\end{equation}
with
\begin{equation}
    \mathbf{F}(\mathbf{U}) =
    \begin{pmatrix}
        \alpha_{j+\frac12}\,\tilde{v}\\[4pt]
        \tilde{p}/\alpha_{j+\frac12}
    \end{pmatrix},
    \qquad
    \alpha_{j+\frac12} = \frac{S_{j+\frac12}}{cS^\ast}.
\end{equation}
The maximum characteristic speed is $\lambda = 1$ in these scaled variables,
independently of $S(x)$.
...
\subsubsection{Right Boundary Condition — Radiation Impedance}
...
The Right boundary model reads:
\begin{equation}
\begin{cases}
...
\subsection{Left Boundary Condition — Reed Dynamics}

At the left boundary $x=0$, the acoustic field is driven by the reed. \\
It is modeled by the nonlinear boundary condition given by the coupled equations~\eqref{eq:1e}--\eqref{eq:1f}.
It's motion is approached as an oscillator driven by the pressure difference between the player's mouth and the interior of the instrument. ($\gamma - p(0,t)$). \\
\begin{equation}
    \begin{cases}
    ...
    \end{cases}
\end{equation}
The outgoing characteristic is
\begin{equation}
    w^- = p(0,t) - v(0,t),
\end{equation}
and the incoming characteristic is determined by imposing $v(0,t) = v_{bc}(t)$,
where $v_{bc}$ is given by equation~\eqref{eq:1e}:
\begin{equation}
...
\begin{equation}
    w^+ = w^- + 2\,v_{bc}(t),
\end{equation}
from which the ghost state is reconstructed as
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.9\textwidth]{dg_solution_const_N800.png}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{dg_convergence_T0.5_euler.png}
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{dg_convergence_T0.5_rk2.png}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{dg_convergence_T0.8_euler.png}
        \caption{Convergence of the DG method with Explicit Euler scheme.}
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{dg_convergence_T0.8_rk2.png}
        \caption{Convergence of the DG method with Rk2 scheme.}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[c]{0.58\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth]{exp_profile.png}
    \end{minipage}
...
\begin{figure}[htbp]
    \centering
    % Remplacez 'figs/dg_solution.png' par le chemin et l'extension réels de votre image (pdf, png, jpg...)
    \includegraphics[width=0.9\textwidth]{dg_solution_exp_N800.png}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[c]{0.58\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth]{cone_profile.png}
    \end{minipage}
...
\begin{figure}[htbp]
    \centering
    % Remplacez 'figs/dg_solution.png' par le chemin et l'extension réels de votre image (pdf, png, jpg...)
    \includegraphics[width=0.9\textwidth]{dg_solution_cone_N800.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{performance_euler.png}
    \caption{Performance study with Explicit Euler time integration.}
    \label{fig:performance_euler}
\end{figure}

The performance results indicate that the execution time scales around $N^2$ for large $N$ values. 
Since the performance study was done in JAX framework, the first run includes
compilation time. This explains the slightly larger (almost a plateau) time for the smallest $N$ values 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{performance_rk2.png}
    \caption{Performance study with RK2 explicit time integration.}
    \label{fig:performance_rk2}
\end{figure}

The performance results for the RK2 explicit time integration scheme also show a scaling of execution time around $N^2$  for large $N$ values 
