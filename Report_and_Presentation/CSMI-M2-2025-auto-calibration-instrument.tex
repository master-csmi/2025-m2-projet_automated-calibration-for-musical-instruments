...
\subsection{Right Boundary conditions}
...
\subsection{Left Boundary Condition}

The left boundary condition is given by the coupling with the reed dynamics, described by equations~(1e)--(1f).
It involves a nonlinear relation between the velocity at the left boundary and the pressure, as well as a second order ODE governing the reed opening $y(t)$.
The reed opening $y(t)$ is governed by the second-order ODE \eqref{eq:1f}, which can be rewritten as a first-order system for numerical integration:
\begin{equation}
\begin{cases}
\partial_t y = z, \\
\partial_t z = \omega_r^2 \left( 1 - y \right) - \frac{\omega_r}{Q_r} z + \varepsilon \omega_r^2 (\gamma - p(0,t)).
\end{cases}
\end{equation}
Note : For now, we will use as initial conditions $y(0)=1$ and $\partial_t y(0)=0$, which corresponds to the reed being initially at rest and fully open.

The equation \eqref{eq:1e} can be rearranged to express the velocity at the left boundary as a function of the pressure and the reed opening:
\begin{equation}    
v(0,t) = \zeta \ell(y)\,F(\gamma - p(0,t)) + \varepsilon\frac{\kappa}{\omega_r}\,z.
\end{equation}

As for the right hand side boundary condition, we consider the characteristic variables at the left boundary:
\begin{equation}
w^- = p - v,
\qquad
w^+ = p + v.
...
The incoming wave $w^-$ is determined by the interior solution, while the outgoing wave $w^+$ is prescribed by the boundary condition.
From the definition of $w^+$ and the expression for $v(0,t)$, we have:
\begin{equation}
\begin{aligned}
w^+
&= p + v \\
&= p + \zeta \ell(y)\,F(\gamma - p) + \varepsilon\frac{\kappa}{\omega_r}\,z.
\end{aligned}
\end{equation}

And by replacing $p$ by $\frac{w^+ + w^-}{2}$, we can express the outgoing wave $w^+$ in terms of the incoming wave $w^-$ and the reed dynamics:
\begin{equation}
\boxed{
w^+ = w^- + 2 \zeta \ell(y)\,F\left(\gamma - \frac{w^+ + w^-}{2}\right) + 2 \varepsilon\frac{\kappa}{\omega_r}\,z       
}
\end{equation}
...
At the left boundary, $x=L$ , the acoustic field interacts with a nonlinear dynamical system
\begin{align}
    -v(0,t)
    + \zeta_\ell(y)\,F(\gamma - p(0,t))
    + \varepsilon \kappa \omega_r \partial_t y(t)
    &= 0,
    \label{eq:bc_left_phys1}
    \\
    \frac{1}{\omega_r^2}\partial_t^2 y
    + \frac{1}{Q_r\omega_r}\partial_t y
    + y - 1
    &= \varepsilon(\gamma - p(0,t)).
    \label{eq:bc_left_phys2}
\end{align}

The first equation defines a dynamic impedance relation between pressure and velocity,
while the second governs the evolution of the mechanical variable $y(t)$.
...
For the righ boundary equations, substituting the characteristic variables into \eqref{eq:bc_pv_phys} yields
\begin{align}
...
\end{equation}
For the left boudary, from \eqref{eq:bc_left_phys1}, the boundary velocity reads
\begin{equation}
    v(0,t)
    =
    \zeta \ell(y)F(\gamma-p(0,t))
    + \frac{\varepsilon\kappa}{\omega_r} \partial_t y.
\end{equation}

The incoming characteristic is therefore
\begin{equation}
    w^+(0,t)=p(0,t)+v(0,t),
\end{equation}
which gives
...
\boxed{
    ...
    p(0,t)
    + \zeta \ell(y)F(\gamma-p(0,t))
    + \frac{\varepsilon\kappa}{\omega_r} \dot y(t).
}
\label{eq:wplus_left}
\end{equation}

Introducing $z=\dot y$, equation \eqref{eq:bc_left_phys2}
